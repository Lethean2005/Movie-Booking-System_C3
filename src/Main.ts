...
// -----------------------------
const user1 = new User('U001', 'Panha', 'panha@gmail.com', 'password123', new Wallet(100));
user1.register();
user1.login();
...
// 3. Add Showtimes
// -----------------------------
const showtime1 = new Showtime('S001', new Date('2025-06-05T19:00:00'), cinema, movie1, hall1);
const showtime2 = new Showtime('S002', new Date('2025-06-05T21:00:00'), cinema, movie2, hall2);
...
// -----------------------------
for (let i = 1; i <= 10; i++) {
    hall1.addSeat(new Seat(`P${i}`, 1, `${i}`, hall1, SeatZone.PREMIUM, SeatStatus.AVAILABLE));
    hall2.addSeat(new Seat(`S${i}`, 1, `${i}`, hall2, SeatZone.STANDARD, SeatStatus.AVAILABLE));
}
...
const actionMovies = cinema.listMovies().filter(m => m.getGenre() === Genre.ACTION);
...
// -----------------------------
console.log('\n------------------- BOOKING & PAYMENT -------------------');
const booking = new Booking('B001', new Date(), BookingStatus.PENDING, 45, selectedShowtime, user1);
